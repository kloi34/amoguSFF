...
-- https://github.com/kloi34/amoguSV
-- amoguSFF not amoguSV
-- replace "sv" with "sff"
...
function centerWindowIfHotkeysPressed()
    ...
    imgui.SetWindowPos("amoguSFF", coordinatesToCenter)
end
--[[
...
function draw()
    ...
    startNextWindowNotCollapsed("amoguSFFAutoOpen")
    focusWindowIfHotkeysPressed()
    centerWindowIfHotkeysPressed()
    
    imgui.Begin("amoguSFF", imgui_window_flags.AlwaysAutoResize)
...
function provideBasicPluginInfo()
    imgui.Text("Steps to use amoguSFF:")
...
function provideMorePluginInfo()
    ...
    linkBox("GitHub repository", "https://github.com/kloi34/amoguSFF")
end
...
function addSVToList(svList, offset, multiplier, endOfList)
    local newSV = utils.CreateScrollSpeedFactor(offset, multiplier)
...
function getRemovableSVs(svsToRemove, svTimeIsAdded, startOffset, endOffset)
  -- map.ScrollSpeedFactors doesn't work yet smh
  --[[
    for _, sv in pairs(map.ScrollSpeedFactors) do
...
function getSVMultiplierAt(offset)
    local sv = map.GetScrollSpeedFactorAt(offset) 
...
function removeAndAddSVs(svsToRemove, svsToAdd)
    ...
    local editorActions = {
        utils.CreateEditorAction(action_type.RemoveScrollSpeedFactorBatch, svsToRemove),
        utils.CreateEditorAction(action_type.AddScrollSpeedFactorBatch, svsToAdd)
    }
...
function deleteSVs()
    ...
    local endOffset = offsets[#offsets]
    local svsToRemove = getSVsBetweenOffsets(startOffset, endOffset)
    if #svsToRemove > 0 then actions.RemoveScrollSpeedFactorBatch(svsToRemove) end
end